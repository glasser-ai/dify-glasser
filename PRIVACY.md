...
- **Tool inputs.** The search query, the provider and endpoint names, the run
  input you build for an endpoint, run ids, and list filters. They are sent to
  the Glasser API solely to fulfil the request they belong to. A run input may
  contain personal data about third parties (for example an email address you
  ask to enrich); the Glasser Privacy Policy covers how Glasser and the
  provider handle it.
...
Tool output is the Glasser API's own response: endpoint listings, endpoint
contracts and prices, run records with the provider's output, and balance
figures. Dify stores that output as part of the conversation or workflow
